...
- 创建集成分支时会写
- 向集成分支添加新的开发分支时会写，且**合并已有 + 新增为一条完整记录**
- **更新集成分支时不会再写**

即：创建时选了 `feature_a`，写 `[DREO-MERGE] dev_xxx <- feature_a`；
后来通过「添加新的开发分支」加入 `feature_b` 时，写的是：
`[DREO-MERGE] dev_xxx <- feature_a,feature_b`（包含全部已集成分支），
而不是单独写一条只含 `feature_b` 的新记录。

之所以这么做，是确保追踪提交始终反映集成分支当前的完整开发分支列表。
更新集成分支时不会再写，避免每次同步新增提交时都插入一条空追踪提交。

### 4.8.1 `[DREO-DESC]` 描述提交规则

当前规则：

- 创建开发分支时，用户必须输入「版本描述」（不可为空）；写入一条空提交 `[DREO-DESC]<描述>` 到开发分支
- 创建集成分支时，会读取所选开发分支最近一条 `[DREO-DESC]` 描述，原样拼接逗号隔开写入集成分支：`[DREO-DESC]<desc1>,<desc2>,...`
- 添加新开发分支到集成分支时，会**合并已有描述 + 新增描述**写入一条完整的 `[DREO-DESC]`（与一次性创建多个分支的结果一致）
- **更新集成分支时不会再写**集成层面的描述提交（避免历史被反复覆盖）
...
- 「添加新开发分支到集成分支」时，必须合并已有描述 + 新增描述写入一条完整的 `[DREO-DESC]`（与一次性创建的结果一致）
- 「更新集成分支」不会写入集成层 `[DREO-DESC]`，避免反复覆盖历史
- 参数模式最后两行输出必须稳定
