...
def note(message, level='info'):
    ...
    print(f"  {paint(icon, color)} {message}")
...
def header(title, icon='🧭', subtitle=None):
    print()
...
def select_one(options, prompt="请选择"):
    ...
    while True:
        raw = input(f"\n  {paint('👉', '36')} {prompt} [0-{len(options)}]: ").strip()
        if raw == '0':
            return None
...
def confirm(prompt):
    return input(f"\n  {paint('❓', '33')} {prompt} (y/n): ").strip().lower() == 'y'
...
def update_integration_branch():
    header("同步更新集成分支（重新合并已集成的开发分支）", icon='🔄')
...
def merge_to_master():
    header("合并发布分支回 master（基线写入）", icon='🚀')
...
def show_status():
    ...
    print()
    print(f"  {paint('🌿', '32')} 当前分支: {branch_text}"
          f"   {paint('🧩', '36')} 开发分支: {len(features)}"
          f"   {paint('🚚', '35')} 集成分支: {len(integrations)}")
...
def run_submenu(title, items):
    ...
    while True:
        show_status()
        sep()
        print(f"  {accent(f'📚 {title}')}")
        for i, (label, _) in enumerate(items, 1):
            print(f"  {accent(f'{i:>2}.')} {label}")
        print(f"  {muted(' 0.')} 返回上一级")
        sep()

        choice = input(f"  {paint('👉', '36')} 请选择: ").strip()
        if choice == '0':
            return False
        if choice in keys[:-1]:
            idx = int(choice) - 1
...
def main():
    ...
    while True:
        ...
        choice = input(f"  {paint('👉', '36')} 请选择操作: ").strip()
        if choice == '0':
            print(f"\n  {paint('👋', '36')} 再见！\n")
            sys.exit(0)
